...
def add_tensor_moment(regionprops_prop, scale):
    ...
    regionprops_prop.tensor_moment = prop_tensor_moment(regionprops_prop, scale)

    return regionprops_prop

def add_tensor_inertia(regionprops_prop, scale):

    regionprops_prop.tensor_inertia = prop_tensor_inertia(regionprops_prop, scale)

    return regionprops_prop

def add_principal_lengths(regionprops_prop, scale, add_principal_vectors: bool = False):
    ...
    if add_principal_vectors:
        ...
        principal_lengths = prop_principal_lengths(
            regionprops_prop, 
            scale=scale,
            return_principal_vectors=False
        )
...
def add_ellipsoidal_coefficients(regionprops_prop, scale):
    ...
    regionprops_prop.ellipsoidal_coefficients = prop_ellipsoidal_coefficients(regionprops_prop, scale)

    return regionprops_prop


def add_ellipsoidal_nature(regionprops_prop, scale, similarity_threshold: float):

    regionprops_prop.ellipsoidal_nature = prop_ellipsoidal_nature(regionprops_prop, scale, similarity_threshold)

    return regionprops_prop

def add_ellipsoidal_nature_bool(regionprops_prop, scale):

    regionprops_prop.ellipsoidal_nature_bool = prop_ellipsoidal_nature_bool(regionprops_prop, scale)
...
def prop_principal_lengths(regionprops_prop, scale, return_principal_vectors: bool = False):
    """
...
def prop_true_strain_tensor(regionprops_prop, scale):
    ...
    if not hasattr(regionprops_prop, 'principal_vectors'):
        regionprops_prop = add_principal_lengths(regionprops_prop, scale, add_principal_vectors=True)
    ...
    tensor = principal_vectors.T @ np.diag(true_strains) @ np.linalg.inv(principal_vectors.T)
...
def prop_ellipsoidal_nature_bool(regionprops_prop, scale):
    ...
    if not hasattr(regionprops_prop, 'ellipsoidal_coefficients'):
        regionprops_prop = add_ellipsoidal_coefficients(regionprops_prop, scale)
...
def prop_ellipsoidal_nature(regionprops_prop, scale, similarity_threshold: float):
