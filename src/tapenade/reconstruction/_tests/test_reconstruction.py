...
def add_random_landmark(array, limits, radius, label):
    x1, y1, z1 = (
        np.random.randint(limits[0], limits[1]),
        np.random.randint(limits[0], limits[1]),
        np.random.randint(limits[0], limits[1]),
    )
    array[
...
def test_manual_registration():
    ...
    limits = [
        20,
        80,
    ]  # landmarks are placed far from the border to avoid going out of the image after rotation
    ...
    array_float = ndi.rotate(
        array_float, rotation_applied[0], axes=(1, 2), order=0, reshape=False
    )
    array_float = ndi.rotate(
        array_float, -rotation_applied[1], axes=(2, 0), order=0, reshape=False
    )
    array_float = ndi.rotate(
        array_float, rotation_applied[2], axes=(0, 1), order=0, reshape=False
    )
    array_float = ndi.shift(
        array_float,
        translation_applied,
        order=0,
        mode="constant",
    ...
    array_float_aftertrans1 = ndi.shift(
        array_float, trans1, order=0, mode="constant", cval=0, prefilter=False
    )
    array_float_rotated = ndi.rotate(
        array_float_aftertrans1, -rot[0], axes=(1, 2), order=0, reshape=False
    )
    array_float_rotated = ndi.rotate(
        array_float_rotated, -rot[1], axes=(2, 0), order=0, reshape=False
    )
    array_float_rotated = ndi.rotate(
        array_float_rotated, -rot[2], axes=(0, 1), order=0, reshape=False
    )
    array_float_registered = ndi.shift(
        array_float_rotated,
        trans2,
        order=0,
        mode="constant",
