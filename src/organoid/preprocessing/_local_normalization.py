...
"""
The code for fast interpolation was taken from the library fast_interp
        ...
        for i in range(2):
            ...
            for j in range(2):
                ...
                for k in range(2):
                    ...
                    # if ixi==f.shape[0] or iyj==f.shape[1] or izk==f.shape[2]:
                    #     print('problem', f.shape,ixi,iyj,izk)
                    # if np.isnan(f[ixi,iyj,izk]*asx[i]*asy[j]*asz[k]):
                    #     print(f.shape,ixi,iyj,izk)
                    #     print('problem', f[ixi,iyj,izk],asx[i],asy[j],asz[k])
...
def local_normalization(image, box_size, perc_low, perc_high):
    """
    Performs local histogram stretching by applying the following steps:
        ...
        3. perform a linear normalization between the interpolated percentile values and the values 0 and 1.
    ...
    grid_positions_and_steps = [np.linspace(0, s-1, n_boxes, retstep=True) for s, n_boxes in zip(array_shape, grid_shape)]
    ...
    for indices_grid, indices_percs in zip(product(*grid_positions), product(*percs_positions)):
        ...
        percs_low[tuple(indices_percs)] = np.percentile(box, perc_low)
        percs_high[tuple(indices_percs)] = np.percentile(box, perc_high)

    full_positions = np.array(np.meshgrid(*[np.arange(s) for s in array_shape], indexing='ij')).reshape(image.ndim, -1)

    # Interpolate percentile values for each voxel
    interp_low = interp3d(a=[0]*image.ndim, b=[s-1 for s in image.shape], 
                          h=grid_steps,f=percs_low,c=[False]*image.ndim)
    
    interp_high = interp3d(a=[0]*image.ndim, b=[s-1 for s in image.shape], 
                          h=grid_steps,f=percs_high,c=[False]*image.ndim)
    ...
    # Perform linear normalization and clip image voxel values
