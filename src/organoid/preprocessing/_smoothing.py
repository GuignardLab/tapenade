...
def _smooth_gaussian(array, mask, sigmas):
    """
    ...
    if mask is None:
        ...
        smooth_array = scipy_gaussian(
            np.where(mask, array*1.0, 0.0), sigmas,
...
def _parallel_gaussian_smooth(input_tuple, sigmas):
    data, mask = input_tuple
    return _smooth_gaussian(data, mask, sigmas)


def gaussian_smooth(image, sigmas, mask=None, n_jobs=-1):
    """
    Apply Gaussian smoothing to an image or a sequence of images.

    Args:
        image (ndarray): The input image or sequence of images.
        sigmas (float or sequence of floats): The standard deviation(s) of the Gaussian kernel.
    ...
    if is_temporal:
        ...
        if n_jobs == 1:
            ...
            iterable = tqdm(zip(image, mask), total=len(image) , desc='Smoothing image')

            return np.array(
                [func(elem) for elem in iterable]
            )
    
        else:
            elems = [elem for elem in zip(image, mask)]

            max_workers = os.cpu_count() if n_jobs == -1 else min(n_jobs, os.cpu_count())
            result = process_map(func, elems, max_workers=max_workers, desc='Smoothing image')
